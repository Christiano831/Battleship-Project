...
confirm.addEventListener("click", function () {
  ...
  info.innerText =
    "Please choose the slots where you would like to place your boats in the lower grid. You can choose to have the boats horizontal or vertical. Just make sure you click the correct amount of slots for the boats.";
...
function enemyBoatGen(boatSize) {
  ...
  if (rowColumn === 0) {
    ...
    let randomColumn =
      columns[Math.floor(Math.random() * (columns.length - (boatSize - 1)))];
    ...
    if (goodBoat === true) {
      ...
      enemyBoatGen(boatSize);
    }
  } else {
    let randomColumn = columns[Math.floor(Math.random() * columns.length)];
    let randomRow =
      rows[Math.floor(Math.random() * (rows.length / 2 - (boatSize - 1)))];
    let boatArray = [randomRow + randomColumn];
    for (let i = 1; i < boatSize; i++) {
...
function enemyTurn() {
  let randomChoice =
    playerButtonsIDs[Math.floor(Math.random() * playerButtonsIDs.length)];
